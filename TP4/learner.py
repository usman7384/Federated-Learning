...
class Learner:
    ...
    __sub__: differentiate learners
...
class Label_Flipping_Learner(Learner):
    """
    A malicious learner that implements label-flipping attacks.
    During training, all labels are shifted by one (modulo the number of classes).
    
    This simulates a Byzantine attack where malicious clients corrupt the training process.
    """
    
    def fit_epoch(self, loader):
        """
        Perform several optimizer steps on all batches drawn from `loader`
        with label-flipping attack: all labels are shifted by one.
        ...
        for x, y in loader:
            ...
            if self.is_binary_classification:
                ...
                # Apply label-flipping attack: shift labels by one
                # For MNIST, this maps 0->1, 1->2, ..., 9->0
