...
def get_malicious_learner(experiment_name, device, optimizer_name, lr, mu, seed):
    """
    constructs malicious learner (Label_Flipping_Learner) for an experiment for a given seed
...
def init_client(args, client_id, client_dir, logger):
    ...
    # Determine if this client should be malicious based on client_id and prop
    # Malicious clients have client_id in range [0, args.prop * 10)
    is_malicious = client_id < int(args.prop * 10)
    
    if is_malicious:
        # Use Label_Flipping_Learner for malicious clients
        learner = get_malicious_learner(
            experiment_name=args.experiment,
            device=args.device,
        ...
        # Use regular Learner for honest clients
        learner = get_learner(
            experiment_name=args.experiment,
            device=args.device,
            optimizer_name=args.local_optimizer,
...
def get_aggregator(
    ...
    ----------
    aggregator_type: str
        possible are {"centralized", "no_communication", "median"}
